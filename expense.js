...
// Flags for our interception logic, not Commander's internal parsing.
const hasHelpFlag = rawArgs.includes("--help") || rawArgs.includes("-h");
const isVersionFlag = rawArgs.includes("--version") || rawArgs.includes("-v");
...
// Generate export labels for filename and PDF title (remains unchanged)
...
// Function to prompt for a choice (now with attempt limit and display)
async function promptChoice(question, choices, maxAttempts = 3) {
  let attempts = 0;
  let choice = null;
  while (choice === null && attempts < maxAttempts) {
    ...
    const currentQuestion = `${question}${chalk.dim(
    ...
    if (choices.includes(answer)) {
      ...
      if (attempts < maxAttempts) {
        // Only show invalid message if more attempts are left
        console.log(
          chalk.yellow(
            `Invalid choice. Please choose one of: ${choices
              .map((c) => `'${c}'`)
              .join(", ")}.`
          )
        );
  ...
  if (choice === null && attempts >= maxAttempts) {
    console.log(
      chalk.red(
        `\nExceeded maximum attempts (${maxAttempts}). Operation cancelled.`
      )
    );
  }
  return choice; // Will be null if max attempts exceeded
}
...
program.option("-h, --help", "display help for command", (value, previous) => {
...
program
  ...
  .action(async (id) => {
    ...
    try {
      ...
      if (!confirmed) {
        console.log(
          chalk.yellow("Operation cancelled. No expense was marked as deleted.")
        );
        return;
      }
...
program
  ...
  .action(async (options) => {
    ...
    try {
      ...
      const tableMarginBottom = 15;
      const footerMargin = 30;
      function checkSpaceForFooter(y, footerMargin, tableMarginBottom) {
        return y - tableMarginBottom < footerMargin;
      }
